...
public class RSPlayer {
   private static final HiscoreClient hiscores = new HiscoreClient(RuneLiteAPI.CLIENT);
   ...
   public RSPlayer(final String userName, final double attackLevel, final double strengthLevel, final double defenseLevel, final double hitpointsLevel, final double rangedLevel) {
      ...
      this.stanceMap.put(DuelType.TENTACLE, this.tentStance);
      this.stanceMap.put(DuelType.RANGED, this.rangeStance);
   }

   @Nullable
   public static RSPlayer fromName(final String name) {
      ...
      try {
         result = RSPlayer.hiscores.lookup(user, HiscoreEndpoint.NORMAL);
   ...
   public static RSPlayer fromStats(int att, int str, int def, int hit, int rng) {
      final String userName = "Manual";
      return new RSPlayer(userName, att, str, def, hit, rng);
   }
